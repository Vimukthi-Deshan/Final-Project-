...
function buildVerifyUrl(batchId) {
    const baseUrl = process.env.PUBLIC_VERIFY_BASE_URL ?? "http://127.0.0.1:5173/verify";
    const separator = baseUrl.includes("?") ? "&" : "?";
    return `${baseUrl}${separator}batchId=${encodeURIComponent(batchId)}`;
}
const batchSchema = z.object({
    sourceSuppliers: z.array(z.object({
...
function generateBatchId(existingBatchIds) {
    const now = new Date();
...
router.get("/batches", async (_req, res) => {
    try {
        ...
        const message = error instanceof Error ? error.message : "Unknown batch list error";
...
router.post("/batches", async (req, res) => {
    try {
        ...
        if (!parsed.success) {
            res.status(400).json(fail("INVALID_BATCH_PAYLOAD", "Batch payload is invalid", {
                issues: parsed.error.issues,
            }));
            return;
        }
        const existingBatches = await listBatches();
        const normalizedBatchId = generateBatchId(existingBatches.map((batch) => batch.batchId));
        ...
        const message = error instanceof Error ? error.message : "Unknown batch create error";
